...
export class DLSAPI{
    ...
    private _logs:DLSLog[]=[]
    private running:boolean=false
    private refreshLoop:AsyncLoop
    set refreshInterval(interval:number){
        //每次修改刷新间隔时，都是直接开启新循环，而不是给旧循环修改时间
        //如果去给旧循环修改间隔，那么修改不会立即生效，因为旧循环还在旧间隔上挂起
        //先停止旧的循环
        this.refreshLoop.stop()
        //然后新建循环给属性赋值，旧循环挂起结束后自动释放
        this.refreshLoop=new AsyncLoop(this.refresh.bind(this),interval)
    ...
    public postProcessStatusRefresh:((newInfo:{
        status:ProcessStatus
    })=>void)[]=[]
    public postHardwareStatusRefresh:((newInfo:{
        status:{}
    })=>void)[]=[]
    ...
    public async refresh(){
        ...
        if(logsAppended.length>0)return;
        const processStatus=await this.refreshProcessStatus()
        //全部刷新完成后触发事件
        this.postProcessStatusRefresh.forEach(fn=>fn(
        {
            status:processStatus
        }))
    }
    public async refreshConsole(){
        ...
        if(旧日志最后一项的新索引!=this._logs.length-1){
            if(旧日志最后一项的新索引>this._logs.length-1)throw new DLSAPIError("算法有误！","BUG")
            else logsAppended=this._logs.slice(旧日志最后一项的新索引+1,this._logs.length)
        }
        const result={
            logsAppended
        }
    ...
    public async terminal_log(log_id:number):Promise<DLSLog[]>{
        const consoleFromTopRaw=await (async()=>{
            try{
                ...
                throw new DLSAPIError("请求"+this.root+"时发生未知错误：\n"+e,"UNKNOWN")
            }
        })()
        //服务器返回404证明路径错误
    ...
    private concatLogs(newLogs:DLSLog[]):{
        ...
        //旧日志为空时值为-1
        let 旧日志最后一项的新索引=this._logs.length-1;
        ...
        try{
            ...
            if(e.code==="OUTDATEDAPIVERSION"){
                ...
                Logger.error("因此，刷新控制台日志的任务已被强制停止。")
                this.stop()
            }
        ...
        for(let i=this._logs.length-1;i>=0;i--){
            ...
            if(newLogs[0].time>=this._logs[i].time){
                新日志开头在旧日志中比哪个地方大=i;
                break;
            }
        }
        ...
        while(newLogs.length>0){
            ...
            if(新日志开头在旧日志中比哪个地方大!=-1){
                ...
                if(
                    this._logs[新日志开头在旧日志中比哪个地方大].time==currentNewLogStart.time
                    &&this._logs[新日志开头在旧日志中比哪个地方大].text==currentNewLogStart.text
                )mergeNewLog=true;//不执行插入操作
            }
            //如果新日志为-1，那么此处第一个参数传入0，目的类似unshift
            if(!mergeNewLog){
                //判断是否在旧日志的最后一项之前插入，如果是，那么需要将旧日志最后一项的新索引向后移一位
                if(新日志开头在旧日志中比哪个地方大<this._logs.length-1){
                    旧日志最后一项的新索引++;
                }
                // Logger.info(`执行splice操作：${新日志开头在旧日志中比哪个地方大+1},0,${currentNewLogStart.text}`)
                this._logs.splice(新日志开头在旧日志中比哪个地方大+1,0,currentNewLogStart)//条件不满足，执行插入
            }
            //如果newLogs插入完这个元素之后自己空了，那么不需要进行下一步，直接退出
            if(newLogs.length==0){
                // Logger.info("已插入完所有日志")
                break;
            }
            //图形化表述：
            //一次具体的插入操作完成后，此时的旧日志：
            ...
            //那么此时它在前一位判断自己比下一位还大，就会跑到数组后面并跑出循环
                ...
                for(;i<this._logs.length-1&&newLogs[0].time>=this._logs[i+1].time;i++){}
                // //原始结构
